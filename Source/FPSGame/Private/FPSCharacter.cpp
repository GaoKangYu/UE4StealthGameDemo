...
{
	//这里需要设置为和头文件一致
	//当在客户端上调用这个函数时，会运行但不在客户端执行，而是发送请求至服务器，内容为请为我运行这一函数
	ServerFire();

	//服务端执行逻辑，客户端需要显示声光效果，服务端不需要
...
//他们魔改cpp是这样的，在generated.h中做了一系列工作
...
{
	//服务器进行完整性检查时用到
	//如果客户端发起return false时服务器会取消连接（使用抓包工具或者出现问题）
	return true;
}
